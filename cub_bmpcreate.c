...
/*   Updated: 2020/10/30 14:02:59 by ajuncosa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		create_bmp(t_vars *vars)
{
	...
	raycasting(vars);
	file_size = 54 + vars->window.width * vars->window.height * (vars->img.bits_per_pixel / 8);
	init_bmp_arrays(vars, file_size);
	write(fd, vars->bmp.file_header, 14);
	write(fd, vars->bmp.info_header, 40);
	...
	while (i >= 0)
	{
		...
		{
			write(fd, &addr[i * vars->window.width + j], 4);
			j++;
		}
		i--;
	}
